...
//    {
//        var exists = await _context.Users.AnyAsync(x =>x.Login.ToLower() == request.Login.ToLower()||x.Email.ToLower() == request.Email.ToLower()||
//            x.Phone == request.Phone||x.PassportNumber == request.PassportNumber);
...
//            RoleId = clientRole.Id, // Всегда роль Client
//            IsActive = true,
//            IsDeleted = false,
...
public class ClientsController : ControllerBase
{
    ...
    public ClientsController(AppDbContext context, AuditService auditService) // ← ДОБАВИТЬ
    {
        _context = context;
